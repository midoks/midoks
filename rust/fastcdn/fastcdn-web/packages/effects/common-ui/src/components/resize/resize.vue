        ...
        case 'b': {
            ...
            if (snapToGrid.value) {
                newBottom =
                    ...
                    Math.round(
                        ((parentHeight.value as number) - newBottom) /
                            gridY.value,
                    ) *
                        gridY.value;
            }
        ...
        case 'r': {
            ...
            if (snapToGrid.value) {
                newRight =
                    ...
                    Math.round(
                        ((parentWidth.value as number) - newRight) /
                            gridX.value,
                    ) *
                        gridX.value;
            }
...
const calcResizeLimits = () => {
    ...
    if (aspectRatio.value) {
        if (minw.value / minh.value > (aspectFactor.value as number)) {
            ...
            minw.value = ((aspectFactor.value as number) *
                minh.value) as number;
        }
    }
    ...
    if (aspectRatio.value) {
        const aspectLimits = {
            left: {
                min:
                    left.value! -
                    Math.min(top.value!, bottom.value!) *
                        aspectFactor.value! *
                        2,
                max:
                    left.value! +
                    ((height.value - minh.value!) / 2) *
                        aspectFactor.value! *
                        2,
            },
            right: {
...
const bodyMove = (delta: { x: number; y: number }) => {
    ...
    if (snapToGrid.value) {
        ...
        let diffR =
            ...
            newRight -
            Math.floor(
                ((parentWidth.value as number) - newRight) / gridX.value,
            ) *
                gridX.value;
