...
export function useHoverToggle(
    ...
    function updateHovers() {
        ...
        isHovers.value = refs.value.map((refEle) => {
            ...
            const hoverRef =
                scope.run(() => useElementHover(eleRef)) || ref(false);
    ...
    function setValueDelay(val: boolean) {
        ...
        if (val) {
            // 鼠标进入
            const enterDelay =
                normalizedOptions.enterDelay ?? DEFAULT_ENTER_DELAY;
            const delayTime = isFunction(enterDelay)
                ? enterDelay()
                : enterDelay;
            ...
            // 鼠标离开
            const leaveDelay =
                normalizedOptions.leaveDelay ?? DEFAULT_LEAVE_DELAY;
            const delayTime = isFunction(leaveDelay)
                ? leaveDelay()
                : leaveDelay;
