...
use pb::ping_service_server::{PingService, PingServiceServer};
    ...
    let result: Result<&str, Box<dyn std::error::Error>> = match &args.command {
        Some(Commands::Start { daemon }) => {
            if *daemon {
                ...
                let current_exe = match std::env::current_exe() {
                    Ok(exe) => exe,
                    Err(e) => {
                        return Err(Box::new(std::io::Error::new(std::io::ErrorKind::Other, "无法获取当前执行文件路径")) as Box<dyn std::error::Error>);
                    }
                };
                ...
                let addr = match "127.0.0.1:50051".parse() {
                      ...
                      Err(e) => {
                          eprintln!("地址解析失败: {}", e);
                          return Err(Box::new(std::io::Error::new(std::io::ErrorKind::InvalidInput, "地址解析失败")) as Box<dyn std::error::Error>);
                      }
                  };
                 ...
                 if let Err(e) = Server::builder()
                      ...
                      return Err(Box::new(std::io::Error::new(std::io::ErrorKind::Other, "gRPC服务器启动失败")) as Box<dyn std::error::Error>);
                  }
        ...
        Some(Commands::Stop {}) => {
            ...
            let pid = match pid_str.trim().parse::<u32>() {
                ...
                Err(_) => {
                    eprintln!("PID文件格式错误");
                    return Err(Box::new(std::io::Error::new(std::io::ErrorKind::InvalidData, "PID文件格式错误")) as Box<dyn std::error::Error>);
                }
            };
