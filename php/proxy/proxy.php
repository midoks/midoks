...
//现有使用地址
//define('NOW_DOMAIN', 'http://midoks.cachecha.com');
include('config.php');
class proxy{
	...
	public function _get(){
		...
		if(isset($_SERVER['REQUEST_URI'])){
			//echo DOMAIN.$_SERVER['REQUEST_URI'];exit;
		...
		curl_setopt($ch, CURLOPT_MAXREDIRS , 20);//设置跳转的次数
		curl_setopt($ch, CURLOPT_HEADER , 0);//头文件
		curl_setopt($ch, CURLOPT_RETURNTRANSFER , 1);//返回数据流
		curl_setopt($ch, CURLOPT_TIMEOUT, 10);
		curl_setopt($ch, CURLOPT_HTTPHEADER , $header);//头文件

		//下为bae的id
		curl_setopt($ch, CURLOPT_PROXY ,'202.108.23.200:80');
