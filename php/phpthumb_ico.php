...
class phpthumb_ico {
	...
	function GD2ICOstring(&$gd_image_array) {
		...
		foreach ($gd_image_array as $key => $gd_image) {
			...
			$BitmapInfoHeader[$key] .= "x00x00x00x00"; // DWORD  biClrUsed; //开源OSPhP.COM.CN
		...
		foreach ($gd_image_array as $key => $gd_image) {
			// ICONDIRENTRY   idEntries[1]; // An entry for each image (idCount of 'em) //OSPHP.COm.CN
			...
			$icondata .= phpthumb_functions::LittleEndian2String($dwBytesInRes, 4); // dwBytesInRes;    // How many bytes in this resource? //开源代码OSPHP.COM.Cn
