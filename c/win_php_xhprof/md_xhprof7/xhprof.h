...
extern uint64 cycle_timer();
extern void hp_trunc_time(struct timeval *tv,uint64 intr);

extern double get_cpu_frequency();

extern long get_us_interval(struct timeval *start, struct timeval *end);
extern void incr_us_interval(struct timeval *start, uint64 incr);
extern double get_us_from_tsc(uint64 count, double cpu_frequency);
extern uint64 get_tsc_from_us(uint64 usecs, double cpu_frequency);

extern uint8 hp_inline_hash(char * str);
extern void hp_array_del(char **name_array);
extern const char *hp_get_base_filename(const char *filename);
